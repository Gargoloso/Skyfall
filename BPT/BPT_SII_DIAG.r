...
ID2 <- ID[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4& !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] # ID of each spectra.

##Extracting coordisates from ID
## Spaxel's coordisates are in the id in format YYXX, with Y = DEC and X = RA.
y <- (trunc(ID[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]/100)  ) #Obtaining Y from id.

x <- (ID[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]-(y*100) ) #Obtaining X from id.

##Extracting line surface specific intensities [1e-16 erg cm^-2 s^-1 A^-1 arcsec^-2] Conditions must be met by all lines.

#H-alpha 6563
FA <-    Fa[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] #Obtaining the[N II] flux density (10^-16 erg/(s cm^2 A pix))
rms_FA <- rms_Fa[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]

#[S II] 6716
FS1 <-    Fsii1[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] #Obtaining the[N II] flux density (10^-16 erg/(s cm^2 A pix))
rms_FS1 <- rms_Fsii1[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]

#[S II] 6731
FS2 <-    Fsii2[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] #Obtaining the[N II] flux density (10^-16 erg/(s cm^2 A pix))
rms_FS2 <- rms_Fsii2[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]

#H-beta 4861
FB <-    Fb[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] #Obtaining the[N II] flux density (10^-16 erg/(s cm^2 A pix))
rms_FB <- rms_Fb[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]

#[O III] 5007
FO <-    Fo[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))] #Obtaining the[N II] flux density (10^-16 erg/(s cm^2 A pix))
rms_FO <- rms_Fo[which(!is.na(AHa) & !is.na(rms_AHa) & AHa <= 4 & !is.na(Fsii1) & !is.na(rms_Fsii1) & !is.na(Fsii2) & !is.na(rms_Fsii2))]
...
#[N II]6584 / H-alpha
sa1 <- log10((FS1 + FS2)/FA)
rms_sa2 <- sqrt(((1/FA)*rms_FS1)^2 + ((1/FA)*rms_FS2)^2 + ((-(FS1+FS2)/FA^2)*rms_FA)^2)  
rms_sa3 <- log10(rms_sa2)
...
type <- seq(1,length(sa),1)
for(jj in 1:length(sa)){
  if(ob[jj] <= mainAGN(sa[jj]) & sa[jj] < 0.05) {type[jj] <- "SF"} #Star Forming spaxels
  else if(ob[jj] <= Kewley06(sa[jj])) {type[jj] <- "LINER"} #LINER-like, uncomment to use Cid Fersandes division and comment the modif line.
...
plot(sa,ob,pch=19, main = galaxy[i], ylab= expression(paste("log"[10]," ([O III] ",lambda,"5007 / H",beta,")")), xlab= expression(paste("log"[10]," ([S II] ",lambda,lambda,"6716,6731 / H",alpha,")")),xlim=c(-1.5,1.5),ylim=c(-1.5,1.5), col="white", cex.lab=1.4, cex.axis=1.4,asp=1)
points(sa[type=="SF"],ob[type== "SF"], col = "blue", pch=19, cex=0.7)
points(sa[type=="LINER"],ob[type=="LINER"], col = "orange", pch=19, cex=0.7)
points(sa[type=="Sy"],ob[type=="Sy"], col = "red", pch=19, cex=0.7)
...
nSy[i] <- length(sa[which(type=="Sy")])
nSF[i] <- length(sa[which(type=="SF")])
nAn[i] <- length(sa[which(type=="LINER")])
