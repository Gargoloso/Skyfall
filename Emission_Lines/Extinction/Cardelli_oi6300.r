...
path_oi <- str_c(galaxy[i],"/lis_oi6300.res") #creates path to Ha and [O I] 6300 data file for each galaxy
...
y <-     (trunc(id[which(fluxhb!=500 & fluxoi6300!=500)]/100)  ) #Obtaining Y from id.
x <-     (id[which(fluxhb!=500 & fluxoi6300!=500)]-(y*100) ) #Obtaining X from id.
...
fb <- fluxhb[which(fluxhb!=500 & fluxoi6300!=500)] #Obtaining the H-alpha flux density (10^-16 erg/(s cm^2 A pix))

#[N II] 6583
print('Extracting [O I] 6300.')
foi6300 <- fluxoi6300[which(fluxhb!=500 & fluxoi6300!=500)] #Obtaining the [O I] 6300 flux density (10^-16 erg/(s cm^2 A pix))

#H-beta dereddened flux.
print('Extracting H-beta dereddened flux.')
Fb <- Fb[which(fluxhb!=500 & fluxoi6300!=500)] #Obtaining 

#Cbeta
print('Extracting H-beta normalization constant.')
Cbeta <- Cbeta[which(fluxhb!=500 & fluxoi6300!=500)]
...
rIoi6300 <- seq(1:length(y))
Foi6300  <- seq(1:length(y))
...
for(j in 1:length(y)){
  ...
  rIoi6300[j] <- ((foi6300[j])/fb[j])*10^(Cbeta[j]*(fb_l[2]-1))
  Foi6300[j]  <- rIoi6300[j]*Fb[j]
}
