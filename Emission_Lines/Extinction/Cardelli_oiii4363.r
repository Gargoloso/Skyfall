...
path_oi <- str_c(galaxy[i],"/lis_hgoiii4363.res") #creates path to Ha and [O III] 4363 data file for each galaxy. This file also contains information from H-gamma, but we won't use it!
...
y <-     (trunc(id[which(fluxhb!=500 & fluxoiii4363!=500)]/100)  ) #Obtaining Y from id.
x <-     (id[which(fluxhb!=500 & fluxoiii4363!=500)]-(y*100) ) #Obtaining X from id.
...
fb <- fluxhb[which(fluxhb!=500 & fluxoiii4363!=500)] #Obtaining the H-alpha flux density (10^-16 erg/(s cm^2 A pix))

#[N II] 6583
print('Extracting [O III] 4363.')
foiii4363 <- fluxoiii4363[which(fluxhb!=500 & fluxoiii4363!=500)] #Obtaining the [O III] 4363 flux density (10^-16 erg/(s cm^2 A pix))

#H-beta dereddened flux.
print('Extracting H-beta dereddened flux.')
Fb <- Fb[which(fluxhb!=500 & fluxoiii4363!=500)] #Obtaining 

#Cbeta
print('Extracting H-beta normalization constant.')
Cbeta <- Cbeta[which(fluxhb!=500 & fluxoiii4363!=500)]
...
rIoiii4363 <- seq(1:length(y))
Foiii4363  <- seq(1:length(y))
...
for(j in 1:length(y)){
  ...
  rIoiii4363[j] <- ((foiii4363[j])/fb[j])*10^(Cbeta[j]*(fb_l[2]-1))
  Foiii4363[j]  <- rIoiii4363[j]*Fb[j]
}
