...
path_base <- str_c(galaxy[i],"/Calzetti_oi6300_Fluxes.dat") #creates path to Ha and [N II]data file for each galaxy
...
y <- trunc(ID[which(!is.na(Foi6300) & !is.na(rms_Foi6300) & (rms_Foi6300/Foi6300) <= 1)]/100) # y coordinate.
x <- ID[which(!is.na(Foi6300) & !is.na(rms_Foi6300) & (rms_Foi6300/Foi6300) <= 1 & (rms_Fsii2/Fsii2) <=1)]-(y*100) # x coordinate.

ID2 <- ID[which(!is.na(Foi6300) & !is.na(rms_Foi6300) & (rms_Foi6300/Foi6300) <= 1 )] # ID of each spectra.
...
# [S II] 6716
FOi6300 <- Foi6300[which(!is.na(Foi6300) & !is.na(rms_Foi6300) & (rms_Foi6300/Foi6300) <= 1)]/2.3504e-11
rms_FOi6300 <- rms_Foi6300[which(!is.na(Foi6300) & !is.na(rms_Foi6300) & (rms_Foi6300/Foi6300) <= 1)] 
...
print('Determining [O I] 6300 luminosity.')
LFoi6300 <- 4*pi*D^2*FOi6300*2.3504e-11 #Luminosity [erg s^-1 ].
...
LFoi6300 <- 10^(remove_outliers(log10(LFoi6300)))
Loi6300  <- LFoi6300[which(!is.na(LFoi6300))]/(asp * 3.826e33) #Convert to Solar luminosity / pc^2.
Xoi6300 <-  X[which(!is.na(LFoi6300))]
Yoi6300  <- Y[which(!is.na(LFoi6300))]
IDoi6300 <-  sprintf("%04d", as.numeric(ID2[which(!is.na(LFoi6300))]))
...
resume <- data.frame(IDoi6300,Loi6300)
tabla <- str_c(galaxy[i],"/",galaxy[i],"_Lum_oi6300.dat")
...
print('Plotting [O I] 6300 luminosity map.')

## Specify color map for z.

rbPal <-  colorRampPalette(rev(rainbow(100,start=0,end=0.7))) 
Col <- rbPal(100)[as.numeric(cut(log10(Loi6300),breaks = 100))]

title <- str_c(galaxy[i]," [O I] 6300 luminosity")
plot(Xoi6300,Yoi6300,xlim=c(-40,40),ylim=c(-40,40),xaxs= "i",yaxs="i", col="white", main = title,xlab = expression(paste(Delta, alpha," (arcsec)")), ylab = expression(paste(Delta, delta," (arcsec)")), cex.lab=1.3, cex.axis=1.3)
#rasterImage(ima,-40,-40,40,40,interpolate=F)
points(Xoi6300,Yoi6300, col = (Col), pch=15,cex = 0.7) #Plots spaxels with color map reversed (to have greater values darker and lower lighter).
grid() #Coordinates'grid.
abline(h = 0, v = 0, col = "gray60",lwd=2)
#mtext(side=3,expression(paste(" log"[10]," H",alpha," Luminosity [erg s"^-1,"]")))
image.plot( -40, -40, legend.only=TRUE, zlim= range(log10(Loi6300)), col = (rev(rainbow(100,start=0,end=0.7)))) #Plots the color bar. Reverse colors here too.

#Saving plot.

map <- str_c(galaxy[i],"/",galaxy[i],"_Luminosity_oi6300.eps")
dev.copy2eps(file=map)
map <- str_c("convert -density 300 ",galaxy[i],"/",galaxy[i],"_Luminosity_oi6300.eps ",galaxy[i],"/",galaxy[i],"_Luminosity_oi6300.png")
system(map)
